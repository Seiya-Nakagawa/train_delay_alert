...
| **APIエンドポイント** | AWS Lambda Function URL | Webサイトからのリクエスト受付 |
| **スケジューリング** | Amazon EventBridge | 定期的なバッチ処理のトリガー |
| **ストレージ** | Amazon S3 | 中間ファイル（ユーザーリスト、遅延情報）のキャッシュ |
| **モニタリング** | Amazon CloudWatch | ログ監視、アラーム通知 |
| **データベース** | Amazon DynamoDB | ユーザー設定の永続化 |
| **通知** | Amazon SNS | 管理者へのイベント通知（エラー、ユーザー登録など） |
| **開発言語** | Python | Lambda関数の実装 |
| **外部サービス** | LINE Messaging API | ユーザー認証、プッシュ通知 |
